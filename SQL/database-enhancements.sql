...
CREATE INDEX IF NOT EXISTS idx_tenants_email ON tenants(email);
  ...
  -- Insert units if they don't exist
  INSERT INTO units (property_id, unit_number, status) 
  SELECT faith_plaza_id, 'Suite 101', 'occupied'
  WHERE NOT EXISTS (SELECT 1 FROM units WHERE unit_number = 'Suite 101' AND property_id = faith_plaza_id);
  
  INSERT INTO units (property_id, unit_number, status) 
  SELECT faith_plaza_id, 'Suite 102', 'occupied'
  WHERE NOT EXISTS (SELECT 1 FROM units WHERE unit_number = 'Suite 102' AND property_id = faith_plaza_id);
  
  INSERT INTO units (property_id, unit_number, status) 
  SELECT faith_plaza_id, 'Suite 103', 'vacant'
  WHERE NOT EXISTS (SELECT 1 FROM units WHERE unit_number = 'Suite 103' AND property_id = faith_plaza_id);
  
  INSERT INTO units (property_id, unit_number, status) 
  SELECT faith_plaza_id, 'Suite 201', 'occupied'
  WHERE NOT EXISTS (SELECT 1 FROM units WHERE unit_number = 'Suite 201' AND property_id = faith_plaza_id);
  
  INSERT INTO units (property_id, unit_number, status) 
  SELECT faith_plaza_id, 'Suite 202', 'occupied'
  WHERE NOT EXISTS (SELECT 1 FROM units WHERE unit_number = 'Suite 202' AND property_id = faith_plaza_id);
  
  -- Get unit IDs
  SELECT id INTO unit_101_id FROM units WHERE unit_number = 'Suite 101' AND property_id = faith_plaza_id;
  SELECT id INTO unit_102_id FROM units WHERE unit_number = 'Suite 102' AND property_id = faith_plaza_id;
  SELECT id INTO unit_201_id FROM units WHERE unit_number = 'Suite 201' AND property_id = faith_plaza_id;
  SELECT id INTO unit_202_id FROM units WHERE unit_number = 'Suite 202' AND property_id = faith_plaza_id;
  
  -- Insert sample tenants with rent due dates for testing automation
  INSERT INTO tenants (unit_id, full_name, email, phone_number, rent_due_date) 
  SELECT unit_101_id, 'John Doe', 'john.doe@email.com', '+234 801 234 5678', CURRENT_DATE + INTERVAL '30 days'
  WHERE NOT EXISTS (SELECT 1 FROM tenants WHERE unit_id = unit_101_id);
  
  INSERT INTO tenants (unit_id, full_name, email, phone_number, rent_due_date) 
  SELECT unit_102_id, 'Jane Smith', 'jane.smith@email.com', '+234 802 345 6789', CURRENT_DATE + INTERVAL '7 days'
  WHERE NOT EXISTS (SELECT 1 FROM tenants WHERE unit_id = unit_102_id);
  
  INSERT INTO tenants (unit_id, full_name, email, phone_number, rent_due_date) 
  SELECT unit_201_id, 'Bob Johnson', 'bob.johnson@email.com', '+234 803 456 7890', CURRENT_DATE + INTERVAL '45 days'
  WHERE NOT EXISTS (SELECT 1 FROM tenants WHERE unit_id = unit_201_id);
  
  INSERT INTO tenants (unit_id, full_name, email, phone_number, rent_due_date) 
  SELECT unit_202_id, 'Alice Brown', 'alice.brown@email.com', '+234 804 567 8901', CURRENT_DATE + INTERVAL '15 days'
  WHERE NOT EXISTS (SELECT 1 FROM tenants WHERE unit_id = unit_202_id);
END $$;
...
BEGIN 
  ...
  RAISE NOTICE 'RLS policies enabled, storage configured, views created, and sample data inserted.';
END $$;